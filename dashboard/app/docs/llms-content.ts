...
For provider_hosted checkout, do not send payment_option_id or payment_method_code. For Xendit, Payment Proxy derives the eligible channels from that installation's ACTIVE payment-method assignments and sends the exact list as allowed_payment_channels; a Xendit checkout with no method eligible for its currency and amount is rejected with NO_ELIGIBLE_PAYMENT_METHOD. Payment Proxy creates the provider's official hosted checkout (Xendit Payment Session, Midtrans Snap, Duitku POP, DOKU Checkout, or iPaymu Redirect Payment) and returns payment.checkout_url plus next_action.redirect_url. Redirect the customer to that provider-owned URL. Emisell must not render its own payment-method page or collect PAN, CVV, OTP, VA, QR, or wallet authorization details.

GET /api/v1/payment-methods?q=<keyword> is the merchant discovery catalog. q is optional, case-insensitive, and searches canonical method code, name, category, and description. Debounce merchant-dashboard input by 300-500 ms. payment-method-assignments configure direct checkout and also restrict Xendit provider-hosted checkout. GET /api/v1/provider-options?environment=sandbox|live groups ACTIVE assignments by ACTIVE provider installation and returns provider identity, installation_id, relative logo_url, and supported_payment_methods; use it when the merchant UI chooses a gateway first. GET /api/v1/payment-options is the flat opaque selection list for the optional direct-channel flow; it takes no environment query and returns ACTIVE options from sandbox and live with environment on every item. GET /api/v1/payment-method-assignments is scoped only by X-Emisell-Merchant-ID, takes no environment query, and returns ACTIVE and INACTIVE records from both sandbox and live with environment on every item. PUT /api/v1/payment-method-assignments accepts {"assignments":[...]} with 1-50 items, applies the batch atomically, derives environment from each installation_id, and must not send X-Emisell-Execution-Mode.
