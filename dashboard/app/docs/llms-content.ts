...
For provider_hosted checkout, do not send payment_option_id or payment_method_code. Payment Proxy derives eligible channels from that installation's ACTIVE payment-method assignments for every provider; a checkout with no eligible method is rejected with NO_ELIGIBLE_PAYMENT_METHOD. Xendit, Midtrans, and DOKU enforce an exact multi-method allowlist. Duitku POP supports hosted checkout only when exactly one method is eligible. iPaymu Redirect Payment has no exact per-transaction allowlist, so the current iPaymu release supports direct checkout only. Payment Proxy returns payment.checkout_url plus next_action.redirect_url for supported provider-owned checkout pages. Emisell must not render its own payment-method page or collect PAN, CVV, OTP, VA, QR, or wallet authorization details.
