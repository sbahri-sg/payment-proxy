...
GET /api/v1/payment-methods?q=<keyword> is the merchant discovery catalog. q is optional, case-insensitive, and searches canonical method code, name, category, and description. Debounce merchant-dashboard input by 300-500 ms. payment-method-assignments configure direct checkout and also restrict Xendit provider-hosted checkout. GET /api/v1/provider-options?environment=sandbox|live groups ACTIVE assignments by ACTIVE provider installation and returns provider identity, installation_id, relative logo_url, and supported_payment_methods; use it when the merchant UI chooses a gateway first. payment-options is the flat opaque selection list for the optional direct-channel request flow. GET /api/v1/payment-method-assignments returns both ACTIVE and INACTIVE records. PUT /api/v1/payment-method-assignments accepts {"assignments":[...]} with 1-50 items, applies the batch atomically, derives environment from each installation_id, and must not send X-Emisell-Execution-Mode.
