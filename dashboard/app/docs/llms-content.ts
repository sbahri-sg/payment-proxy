...
X-Emisell-Execution-Mode: sandbox | live is required only by endpoints that explicitly document it, such as integration-readiness and internal diagnostics. Payment Methods and Payment Sessions do not use this header. Payment Sessions derive environment from payment_method_id (provider-hosted) or payment_option_id (direct); installation mutations derive it from installation_id. Payment-method-assignments, payment-options, and provider-options return both environments. Payment-session listing accepts an optional environment query.
...
Header: Idempotency-Key is required. Do not send X-Emisell-Execution-Mode; Payment Proxy derives environment and installation from payment_method_id or payment_option_id.
Preferred request fields: payment_method_id, checkout_mode=provider_hosted, merchant_reference, amount as whole rupiah for IDR, currency, customer, return_url, metadata. Obtain payment_method_id from supported_payment_methods[].id in GET /api/v1/provider-options. IDR 10000 means exactly Rp10.000.

For provider_hosted checkout, send one opaque payment_method_id (pmo_...) and do not send installation_id, payment_option_id, or payment_method_code. Payment Proxy resolves the owning installation, validates the assignment and availability, and enforces exactly that selected method on the provider checkout. Xendit, Midtrans, DOKU, and Duitku support this exact selection. iPaymu Redirect Payment has no exact per-transaction allowlist, so the current iPaymu release supports direct checkout only. Payment Proxy returns payment.checkout_url plus next_action.redirect_url for supported provider-owned checkout pages. Emisell must not render its own provider payment page or collect PAN, CVV, OTP, VA, QR, or wallet authorization details.

GET /api/v1/payment-methods?q=<keyword> is the merchant discovery catalog. q is optional, case-insensitive, and searches canonical method code, name, category, and description. Debounce merchant-dashboard input by 300-500 ms. payment-method-assignments configure direct checkout and restrict every supported provider-hosted checkout. GET /api/v1/provider-options groups currently available ACTIVE assignments by ACTIVE provider installation and returns sandbox plus live in one response; use each item's environment field. Providers and channels reported offline or maintenance are omitted by Payment Proxy. GET /api/v1/payment-options is the flat opaque selection list for the optional direct-channel flow and applies the same internal availability filter. Do not display an outage state in merchant settings: GET /api/v1/payment-method-assignments remains configuration-only and returns ACTIVE and INACTIVE records unchanged. PUT /api/v1/payment-method-assignments accepts {"assignments":[...]} with 1-50 items, applies the batch atomically, derives environment from each installation_id, and must not send X-Emisell-Execution-Mode. POST /api/v1/payment-sessions returns 503 when the selected provider or direct method is temporarily unavailable and never automatically fails over to another provider.
