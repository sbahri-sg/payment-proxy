...
provider-hosted checkout. Duitku POP exposes only one `paymentMethod` selector,
so hosted checkout is available when exactly one eligible assignment is
`ACTIVE`; larger sets fail closed instead of exposing inactive methods.
...
`emisell-duitku-v2.0.2`; an upload never creates one container per merchant.
