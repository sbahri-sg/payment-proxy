...
func main() {
	...
	if err := run(logger); err != nil {
		logger.Error("Duitku Provider App stopped", "error", err)
		os.Exit(1)
	}
}

func run(logger *slog.Logger) error {
	cfg, err := config.LoadDuitkuProviderApp()
	if err != nil {
		return err
	}
	providerConnector, err := duitku.New(
		cfg.SandboxPOPBaseURL,
		cfg.LivePOPBaseURL,
		cfg.SandboxAPIBaseURL,
		cfg.LiveAPIBaseURL,
		cfg.ConnectorTimeout,
	)
	if err != nil {
		return err
	}
	...
	go func() {
		logger.Info("Duitku Provider App started", "address", server.Addr, "version", providerConnector.Manifest().Version, "executable_sha256", digest)
