...
func main() {
	...
	if err := run(logger); err != nil {
		logger.Error("DOKU Provider App stopped", "error", err)
		os.Exit(1)
	}
}

func run(logger *slog.Logger) error {
	cfg, err := config.LoadDokuProviderApp()
	if err != nil {
		return err
	}
	providerConnector, err := doku.New(cfg.SandboxBaseURL, cfg.LiveBaseURL, cfg.ConnectorTimeout)
	if err != nil {
		return err
	}
	...
	go func() {
		logger.Info("DOKU Provider App started", "address", server.Addr, "version", providerConnector.Manifest().Version, "executable_sha256", digest)
