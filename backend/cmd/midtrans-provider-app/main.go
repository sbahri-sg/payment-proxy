...
func main() {
	...
	if err := run(logger); err != nil {
		logger.Error("Midtrans Provider App stopped", "error", err)
		os.Exit(1)
	}
}

func run(logger *slog.Logger) error {
	cfg, err := config.LoadMidtransProviderApp()
	if err != nil {
		return err
	}
	providerConnector, err := midtrans.New(cfg.SandboxBaseURL, cfg.LiveBaseURL, cfg.ConnectorTimeout)
	if err != nil {
		return err
	}
